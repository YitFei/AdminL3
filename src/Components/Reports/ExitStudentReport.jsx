...
export default function DataGridVIPStudent(props) {
  ...
  React.useEffect(() => {
    API(
      "Get",
      "/student/admin/get/exitStudentList",
      localStorage.getItem("token")
    ).then((res) => {
      let students = res.data.map((s) => {
        let unix_timestamp_dob = s.dob;
        let unix_timestamp_createAt = s.createAt;

        var dateDOB = new Date(unix_timestamp_dob);
        var dateCreateAt = new Date(unix_timestamp_createAt);

        return {
          ...s,
          dob: formatDate(dateDOB),
          createAt: formatDate(dateCreateAt),
        };
      });
  ...
  const columns = [
    ...
    { field: "classInSchool", headerName: "班级", width: 200 },
    {
      field: "name",
      headerName: "名字",
